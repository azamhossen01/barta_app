                ...
                <div class="hidden sm:ml-6 sm:flex gap-2 sm:items-center">
                    ...
                    <div class="relative ml-3" x-data="{ open: false }">
                        <div>
                            <button @click="open = !open" type="button"
                                ...
                                <span  class="absolute top-0 right-0 bg-red-500 text-white px-2 py-1 rounded-full text-xs font-bold">{{ auth()->user()->unreadNotifications()->count() }}</span>
                            </button>
                        </div>
                        ...
                        <div x-show="open" @click.away="open = false"
                            ...
                            @forelse (auth()->user()->notifications as $notification)
                              <a href="{{ route('notification.seen', $notification) }}"
                              class="{{ $notification->read_at ? '' : 'bg-gray-200' }} my-2 block px-4 py-2 text-sm text-gray-700 hover:bg-gray-500" role="menuitem"
                              tabindex="-1" id="user-menu-item-0">{{ $notification->data['message'] }}
                            <small class="block">{{ $notification->created_at->diffForHumans() }}</small>
                            </a>
                    ...
                    <div class="relative ml-3" x-data="{ open: false }">
                        ...
                        <div x-show="open" @click.away="open = false"
                            ...
                            role="menu" aria-orientation="vertical" aria-labelledby="user-menu-button" tabindex="-1">
                            @auth
                            <a href="{{ route('author_profile', Crypt::encrypt(Auth::user()->username)) }}"
